describe('Code duplication bad practice - Sample 1', () => {
  beforeEach(()=>{
    ...
    cy.wait('@getStories')
    cy.get('input[type="text"]').should('be.visible').and('have.value', 'redux').as('searchField')
  })

  it('searches by typing and hitting enter', () => {
    cy.get('@searchField').clear().type('frontend testing{enter}')
    cy.wait('@getStories')
    cy.get('.table-row').should('have.length', 100)
  })

  it('searches by typing and pressing the search button', () => {
    cy.get('@searchField').clear().type('frontend testing')
